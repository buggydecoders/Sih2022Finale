...
const RequirementCard = ({ data, id, selected, setSelected }) => {
    const isSelected = id === selected;

    return (
        <div key={id} onClick={() => setSelected(id)} className={`${isSelected ? 'shadow-xl scale-[1.03] -translate-x-[10px]' : 'hover:shadow-xl hover:scale-[1.03] hover:-translate-x-[10px]'} bg-white rounded-md w-[400px] py-5 shadow-sm  transition-all cursor-pointer px-4`}>
