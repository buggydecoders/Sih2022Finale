...
const MessageController = ({sendMessage})=>{
  ...
  return (
    <div className='w-full bg-transparent flex items-center py-2 rounded-xl gap-3'>
      ...
      <input onKeyUp={(e)=>e.key==='Enter'&&handleSend()} value={text} onChange={(e)=>setText(e.target.value)} className=' border-[1px] border-gray-300  w-full px-3 py-2 text-sm rounded-2xl outline-none' placeholder='Type your Message here!'/>
      </div>
