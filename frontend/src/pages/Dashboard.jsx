...
const Dashboard = () => {
  ...
  const SharedResourceCard = ({ data }) => {
    return (
      ...
      >
        ...
        <div className="flex flex-col">
          <p className="text-sm font-semibold">
            {data?.resource?.name?.substr(0, 20)}{" "}
            {data?.resource?.name?.length < 20 ? "" : "..."}
          </p>
          <div className="flex gap-2">
            <div className="flex gap-2 items-center text-xs">
  ...
  const MessageCard = ({ data }) => {
    ...
    let cardUserData =
      data?.users[0]._id === user._id ? data?.users[1] : data?.users[0];

    let currentLastMessage = isActive ? lastMessage : data?.lastMessage;
    let updatedLastMessage = isActive
      ? lastMessage?.content
      : data?.lastMessage?.content || "";
    if (updatedLastMessage?.length > 17)
      updatedLastMessage = updatedLastMessage?.slice(0, 17) + "..";
    return (
      <div
        className={`${
          isActive ? "" : ""
        } border-b-[1px] cursor-pointer relative border-black border-opacity-10 px-1 py-5`}
      >
        {isActive && (
          <div className="w-[5px] h-[85%] bg-primary absolute top-[50%] -translate-y-[50%] -left-2"></div>
        )}
        <div className="flex gap-3 items-center">
          <img
            src={cardUserData?.logo}
            alt=""
          ...
          <div className=" text-sm w-full">
            <div className="flex flex-col w-full">
              <div className="font-semibold text-xs ">
                {cardUserData?.instituteName?.slice(0, 12)}...
              </div>
              <div className="text-xs font-[500]  text-gray-500">
                {moment(currentLastMessage.createdAt).diff(
                  moment(),
                  "minutes"
                ) * -1}{" "}
                min ago
            ...
            <div className="mt-1 flex items-center justify-between">
              ...
              <div className="h-[17px] w-[17px] text-white  bg-primary flex items-center justify-center text-xs rounded-full">
                1
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <CardCollection viewAll="/inbox" redirect="/inbox" title="Enquries">
              {(!rooms || rooms?.length === 0) && (
                <div className="py-20 text-center w-full font-open text-gray-500 font-[500]">
                  No Chats Found
                </div>
              )}
              {rooms?.length > 0 &&
